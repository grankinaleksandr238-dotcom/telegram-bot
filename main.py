...
async def profile_handler(message: Message):
    user_id = message.from_user.id
    if not await check_subscription(user_id):
        await message.answer("❗️ Сначала подпишитесь на каналы.", reply_markup=subscription_inline())
        return
    async with aiosqlite.connect(DB_PATH) as db:
...
async def buy_callback(callback: CallbackQuery):
    ...
    if not await check_subscription(user_id):
        await callback.message.edit_text("❗️ Сначала подпишитесь на каналы.", reply_markup=subscription_inline())
        return
...
async def casino_bet_amount(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        await message.answer(
            "Главное меню:",
            reply_markup=user_main_keyboard(is_admin(message.from_user.id))
        )
        return
    try:
    ...
    await message.answer(
        ...
        reply_markup=user_main_keyboard(is_admin(user_id))
    )
    await state.clear()
...
async def list_channels(message: Message):
    if not is_admin(message.from_user.id):
        return
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT chat_id, title FROM channels") as cursor:
            channels = await cursor.fetchall()
    if not channels:
